...
def write_noise_pwl(noise_model: 'NoiseModel',
                    time: np.ndarray,
                    I_ph: float,
                    bandwidth: float,
                    filename,
                    random_seed: Optional[int] = None,
                    max_points: int = 10000) -> str:
    """
    Write calibrated noise current PWL file for SPICE injection.

    Generates time-domain noise samples using the full 6-source NoiseModel
    and writes them as a PWL current source file. The noise is injected
    at the INA input (current domain) to match the physical noise point.

    Args:
        noise_model: Configured NoiseModel instance
        time: Time array (s)
        I_ph: Average photocurrent for noise level (A)
        bandwidth: Noise bandwidth (Hz)
        filename: Output file path
        random_seed: RNG seed for reproducibility (None = random)
        max_points: Maximum PWL points (downsampled for SPICE efficiency)
    ...
    # Generate full noise samples
    noise = noise_model.generate_time_domain(len(time), I_ph, bandwidth, rng=rng)

    # Downsample for SPICE efficiency
    step = max(1, len(time) // max_points)
    t_ds = time[::step]
    n_ds = noise[::step]

    # Compute noise stats for header
    breakdown = noise_model.compute_noise(abs(I_ph), bandwidth)

    with open(filename, 'w') as f:
        f.write("; Calibrated noise current PWL for SPICE injection\n")
        f.write("; Generated by cosim.pwl_writer.write_noise_pwl\n")
        f.write(f"; Noise model: {noise_model}\n")
        f.write(f"; Total noise std: {breakdown.total_std:.4e} A\n")
        f.write(f"; Shot:    {np.sqrt(breakdown.shot):.4e} A rms\n")
        f.write(f"; Thermal: {np.sqrt(breakdown.thermal):.4e} A rms\n")
        f.write(f"; Ambient: {np.sqrt(breakdown.ambient):.4e} A rms\n")
        f.write(f"; Amp:     {np.sqrt(breakdown.amplifier):.4e} A rms\n")
        f.write(f"; I_ph avg: {I_ph:.4e} A\n")
        f.write(f"; Bandwidth: {bandwidth:.0f} Hz\n")
        if random_seed is not None:
            f.write(f"; Random seed: {random_seed}\n")
        f.write(f"; Points: {len(t_ds)} (from {len(time)} original)\n")
        f.write(";\n")

        for t, n in zip(t_ds, n_ds):
            f.write(f"{t:.10e} {n:.10e}\n")
